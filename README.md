...
Данная страница отображает базовый (base.html) шаблон, который дополняется блоками из шаблона страницы аутентификации (mainpage.html).
На Рисунке 2 показано в каком виде пользователю представляется страница.
На Главной (Домашней) странице отображается название сайта или приложения (Authentications), а также форма регистрайции, в которой можно:
  -	Выбрать метод (session-based authentication, JWT или OAuth);
